...
import org.aja.tej.utils.{CommandLineOptions, FileUtils}
...
object Multiplication {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      //SparkContext.parallelize to process each row in parallel (subject to the available cores on the machine or
      // cluster, of course). In this case, we sum the values in each row and compute the average.
      val sums_avgs = sc.parallelize(1 to dimensions.m).map { i =>
        ...
        val sum = matrix(i-1) reduce (_ + _) //Reducing the Array or adding Array coponents
      ...
      // Scala's Vector, not MLlib's version!
      val outputLines = Vector(s"${dimensions.m}x${dimensions.n} Matrix:") ++ sums_avgs.zipWithIndex.map {
        case ((sum, avg), index) =>
