...
object StringUtils {
  ...
  def byteToHexString(bytes: Byte): String = {
   ...
   * @param str
   */
  def stringToURI(str: Array[String]): Array[URI] = {
    if (str == null) return null
    val uris: Array[URI] = new Array[URI](str.length)
    {
      var i: Int = 0
      while (i < str.length) {
        {
  ...
  val emptyStringArray: Array[String] = Arra
  )
  val COMMA: Char = ','
  ...
  object TraditionalBinaryPrefix extends Enumeration {
     ...
     */
    def valueOf(symbol: Char): StringUtils.TraditionalBinaryPrefix = {
    ...
    def string2long(s: String): Long = {
      ...
      if (Character.isDigit(lastchar)) return Long.parseLong(s)
        ...
        val num: Long = Long.parseLong(s.substring(0, lastpos))
  ...
  final class TraditionalBinaryPrefix {
    final val value: Long = 0L
    final val symbol: Char = 0
