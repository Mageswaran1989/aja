...
object MultiDimensionalSet {
  def hashSet[K, V]: MultiDimensionalSet[K, V] = {
    return new MultiDimensionalSet[K, V](new HashSet[Nothing])
  }

  def treeSet[K, V]: MultiDimensionalSet[K, V] = {
    return new MultiDimensionalSet[K, V](new TreeSet[Nothing])
  }

  def concurrentSkipListSet[K, V]: MultiDimensionalSet[K, V] = {
    return new MultiDimensionalSet[K, V](new ConcurrentSkipListSet[Nothing])
  }
}

class MultiDimensionalSet[K, V] extends Set[Nothing] {
  private var backedSet: Set[Nothing] = null

  def this(backedSet: Set[Nothing]) {
    this()
  ...
  def contains(k: K, v: V): Boolean = {
    return contains(new Nothing(k, v))
  }
