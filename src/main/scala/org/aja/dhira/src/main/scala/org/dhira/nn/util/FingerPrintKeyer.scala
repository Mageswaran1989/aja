 ...
 *
 */
object FingerPrintKeyer {
  private[util] val punctctrl: Pattern = Pattern.compile("\\p{Punct}|[\\x00-\\x08\\x0A-\\x1F\\x7F]")
}

class FingerPrintKeyer {
  def key(ss: String, o: AnyRef*): String = {
    var s = ss
    ...
    s = s.trim
    s = s.toLowerCase
    s = FingerPrintKeyer.punctctrl.matcher(s).replaceAll("")
    val frags: Array[String] = StringUtils.split(s)
    val set: TreeSet[String] = new TreeSet[String]
    Collections.addAll(set, frags)
    val b: StringBuilder = new StringBuilder
    val i: Iterator[String] = set.iterator
    while (i.hasNext) {
      b.append(i.next)
      if (i.hasNext) {
        b.append(' ')
  ...
  protected def asciify(s: String): String = {
    val c: Array[Char] = s.toCharArray
    val b: StringBuilder = new StringBuilder
    for (element <- c) {
   ...
   */
  private def translate(c: Char): Char = {
    c match {
      case '\u00C0' =>
      case '\u00C1' =>
      case '\u00C2' =>
