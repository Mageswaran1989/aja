...
object MultiDimensionalMap {
   ...
   * @return
   */
  def newThreadSafeTreeBackedMap[K, T, V]: MultiDimensionalMap[K, T, V] = {
    return new MultiDimensionalMap[K, T, V](new ConcurrentSkipListMap[Nothing, V])
  }
...
class MultiDimensionalMap[K, T, V] extends Serializable {
  private var backedMap: Map[Nothing, V] = null

  def this(backedMap: Map[Nothing, V]) {
    this()
  ...
  override def equals(o: AnyRef): Boolean = {
    ...
    if (!(o.isInstanceOf[MultiDimensionalMap[_, _, _]])) return false
    val that: MultiDimensionalMap[_, _, _] = o.asInstanceOf[MultiDimensionalMap[_, _, _]]
    return !(if (backedMap != null) !(backedMap == that.backedMap) else that.backedMap != null)
  }
