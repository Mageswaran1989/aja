...
//TODO: Get overall usage of this class and try to redesign from sratch
package org.dhira.core.util

import org.apache.commons.io.FileUtils
import org.apache.commons.io.IOUtils
import org.dhira.core.containers.Counter
import org.dhira.core.containers.StringUtils
...
import org.dhira.core.containers.StringUtils._
...
object StringGrid {
  ...
  def fromFile(file: String, sep: String): StringGrid = {
    val r = FileUtils.readLines(new File(file))
    val read: List[String] = Source.fromFile(file).getLines().toList
    if (read.isEmpty) throw new IllegalStateException("Nothing to read; file is empty")
    return new StringGrid(sep, read)
  }

  @throws(classOf[IOException])
  def fromInput(is: InputStream, sep: String): StringGrid = {
    val read: List[String] = Source.fromInputStream(is).getLines().toList
    if (read.isEmpty) throw new IllegalStateException("Nothing to read; file is empty")
    return new StringGrid(sep, read)
  }
}
...
@SerialVersionUID(4702427632483221813L)
class StringGrid extends ArrayBuffer[List[String]] {
  private var sep: String = null
  private var numColumns: Int = -1


  def this(sep: String, iteratableData: Iterable[String]) {
    this()
    this.sep = sep
    val data = iteratableData.toList
    for( i <- 0 until data.size) {
      val line: String = data(i).trim
      if (line.indexOf('\"') > 0) {
        val counter: Counter[Character] = new Counter[Character]
        for (j <- 0 until line.length) {
          counter.incrementCount(line.charAt(j), 1.0)
        }
        if (counter.getCount('"') > 1) {
          val split: List[String] = splitOnCharWithQuoting(line, sep.charAt(0), '"', '\\').toList
          +=(split) // add(new ArrayBuffer[String](Arrays.asList(split)))
        }
        else {
          val row: List[String] = (splitOnCharWithQuoting(line, sep.charAt(0), '"', '\\')).toList
  ...
  def removeRowsWithEmptyColumn(column: Int) {
    val remove: List[List[String]] = List()
//    import scala.collection.JavaConversions._
    for (list <- this) {
